...
class _CartPageState extends State<CartPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (user == null) {
      return Scaffold(
        ...
        body: Center(child: Text('Please log in to view your cart', style: TextStyle(fontSize: 16))),
      );
    }
    ...
    return Scaffold(
      ...
      body: StreamBuilder(
        ...
        builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
          if (!snapshot.hasData) {
            return Center(child: CircularProgressIndicator(color: Color(0xFF1DCFCA)));
          }

          final cartItems = snapshot.data!.docs;

          if (cartItems.isEmpty) {
            return Center(child: Text('Your cart is empty', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500)));
          }
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (data == null) {
                      return Padding(
                        ...
                        child: Text("Invalid item - missing data", style: TextStyle(color: Colors.red)),
                      );
                    }
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      IconButton(
                                        icon: Icon(Icons.remove, color: Color(0xFF1DCFCA)),
                                        onPressed: () => updateCartItem(user.uid, item.id, false),
                                      ),
                                      Text(
                                      ...
                                      IconButton(
                                        icon: Icon(Icons.add, color: Color(0xFF1DCFCA)),
                                        onPressed: () => updateCartItem(user.uid, item.id, true),
                                      ),
                                    ],
                                  ),
                                ],
  ...
  void updateCartItem(String userId, String itemId, bool isIncrement) {
    DocumentReference cartItemRef =
        _firestore.collection('users').doc(userId).collection('cart').doc(itemId);
